...
def evaluate_pruning_model(model, dataloaders, dataset_sizes, config, device):
    """
...
def eval_saliency_dataloaders(model, layers, data_loader, activation, device, config, pruned=None):
    ...
    logger.info('Starting evaluation of gradient-based bias influence (saliency scores)...')
    ...
    logger.info('Running dummy forward pass to infer feature map dimensions...')
    __ = model(torch.zeros((1, 3, 224, 224), device=device, dtype=torch.float))

    logger.info('Counting total number of prunable structures across all layers...')
    for i_l, l in enumerate(layers):
        ...
        if isinstance(l, nn.Linear):
            ...
            logger.info(f'Layer {i_l}: Linear with {l.out_features} prunable units.')
    ...
    logger.info(f'Total number of prunable structures: {total_n_structs}')

    # Forward through the model to get activations
    # NOTE: this is a batched version of the algorithm presented in the paper
    if pruned is not None:
        pruned_array = np.array(pruned)
    layer_keys = ['l' + str(i) for i in range(len(layers))]
    logger.info('Starting batched evaluation over data loader...')
    for batch_idx, (inputs, labels, attrs) in enumerate(tqdm(data_loader, desc='Evaluating Saliency', leave=False)):
        model.zero_grad()
        ...
        with autocast('cuda',enabled=(device.type == 'cuda')):
            if pruned is None:
                ...
                outputs = model(X, pruned=pruned_array)
        ...
        if not torch.isnan(bias_measure):
            ...
            for i, layer_key in enumerate(layer_keys):
                if isinstance(layers[i], nn.Linear):
                    coeffs[start_idx[i]:end_idx[i]] += torch.mean(activation[layer_key].grad, dim=0).detach().cpu().numpy()
                elif isinstance(layers[i], nn.Conv2d):
                    coeffs[start_idx[i]:end_idx[i]] += torch.mean(activation[layer_key].grad, 0).detach().cpu().numpy().flatten()
                else:
...
def prune(model, layer_map, data_loader_train, data_loader_val, dataset_size_val, config, seed, device, arch='vgg',
    ...
    with torch.no_grad():
        ...
        cnt = 0
        for X_, y_, p_ in tqdm(data_loader_val, desc='Initial Validation', leave=False):
            X_ = X_.to(device)
            ...
            start = cnt * config['pruning']['batch_size']
            end = (cnt + 1) * config['pruning']['batch_size']
            valid_pred_scores[start:end] = outputs[:, 0].cpu().numpy()
            y_valid[start:end] = y_.cpu().numpy()
            p_valid[start:end] = p_.cpu().numpy()

            cnt += 1

        # Choose the best threshold w.r.t. the balanced accuracy on the held-out data
        logger.info('Choosing best threshold on initial validation scores...')
    ...
    prune_bar = tqdm(total=int((len(struct_inds) + 1) / config['pruning']['step_size']), desc='Pruning Steps')
    with torch.no_grad():
        ...
        for j in range(0, len(struct_inds), config['pruning']['step_size']):
            ...
            logger.info(f'Pruning step {j // config["pruning"]["step_size"]}: Pruning {len(to_prune)} structures.')
            if not config['pruning']['dynamic']:
            ...
            for X_, y_, p_ in tqdm(data_loader_val, desc=f'Validation after {cnt} Prunes', leave=False):
                model.zero_grad()
                X_ = X_.to(device)
                y_ = y_.to(device).to(torch.float)
                p_ = p_.to(device)
                ...
                start = cnt_ * config['pruning']['batch_size']
                end = (cnt_ + 1) * config['pruning']['batch_size']
            ...
            if config['pruning']['stop_early'] and step_num >= config['pruning']['max_steps']:
                logger.info('Early stopping: Max pruning steps reached.')
                prune_bar.close()
                break
        ...
        logger.info('Removing all forward hooks after pruning.')
